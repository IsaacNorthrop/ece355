...
void myEXTI_Init() // *WRITTEN*
{
	/* Map EXTI2 line to PA2 */
	// Relevant register: SYSCFG->EXTICR[0]
	// might need to be EXTICR1
	SYSCFG->EXTICR[0] = ((uint32_t)0x00000000);
	//
	/* EXTI2 line interrupts: set rising-edge trigger */
	// Relevant register: EXTI->RTSR
	EXTI->RTSR |= ((uint32_t)0x00000004); // either needs the initial bit to be 1 or the TR2 bit to be one

	/* Unmask interrupts from EXTI2 line */
	// Relevant register: EXTI->IMR
	EXTI->IMR |= ((uint32_t)0x00000004);

	/* Assign EXTI2 interrupt priority = 0 in NVIC */
	// Relevant register: NVIC->IP[2], or use NVIC_SetPriority
	//NVIC->IP[2] = ((uint32_t)0x00FFFFFF); // according to lab1 code this is right
	NVIC_SetPriority(EXTI2_3_IRQn, 0);
//	NVIC->IP[2] = ((uint32_t)0xFFFF00FF);

	/* Enable EXTI2 interrupts in NVIC */
	// Relevant register: NVIC->ISER[0], or use NVIC_EnableIRQ
	//NVIC->ISER[0] =((uint32_t)0x00008000); //according to lab1 code this is right
...
{ //we might also have to use event updates to count how many edges
	...
	if ((EXTI->PR & EXTI_PR_PR2) != 0)
	{
		...
		// might need to stop timer here, might not matter tho
